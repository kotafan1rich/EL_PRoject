...
class UserDAL:
	...
	async def update_user_by_tg_id(self, id_tg: int, **kwargs) -> Union[User, None]:
		query = (
			update(User).where(User.id_tg == id_tg).values(kwargs).returning(User.id_tg)
		)
		res = await self.db_session.execute(query)
	...
	async def delete_user_by_tg_id(self, id_tg: int) -> Union[int, None]:
		query = delete(User).where(User.id_tg == id_tg).returning(User.id_tg)
		res = await self.db_session.execute(query)
